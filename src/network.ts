...
export class Network {
	...
	public predict(input: number[] | number[][][]): number[] | number[][][] {
	...
	public gradient(batch: Sample[]): DenseLayerParameters[] {
		const gradientSum: DenseLayerParameters[] = this.trainableLayers.map(layer => ({
			weights: Array.from({length: (layer.outputShape as number)},
				() => Array((layer.inputShape as number)).fill(0)),
			biases: Array((layer.outputShape as number)).fill(0)
		}))
		...
		for (const sample of batch) {
			...
			const singleGradient: DenseLayerParameters[] = [
				...hiddenLayersGradients.reverse(),
				outputLayerGradients
			]

			singleGradient.forEach((layer: DenseLayerParameters, idx: number) => {
	...
	public applyGradient(gradient: DenseLayerParameters[]): void {
		// only for adam
	...
	public sgd(
		...
		for (let i = 0; i < numBatches; i++) {
			...
			const gradient: DenseLayerParameters[] = network.gradient(batch);
